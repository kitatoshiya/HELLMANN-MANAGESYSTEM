/**
 * Core Microsoft Graph API integration logic for Microsoft 365 Exchange Online
 * Shared across Express server (Cloud Run / Local Dev) and Vercel Serverless Functions
 */
...
/**
 * Handler for /api/m365/users
 */
export async function handleM365Users(reqBody: any): Promise<{ status: number; data: any }> {
  try {
    const { tenantId, clientId, clientSecret } = reqBody || {};
    ...
    if (!actualTenantId || !actualClientId || !actualClientSecret) {
      return { status: 400, data: { success: false, error: '認証情報が不足しています。' } };
    }
    ...
    return { status: 200, data: { success: true, users: combined } };
  } catch (err: any) {
    return { status: 500, data: { success: false, error: err.message || 'ユーザー一覧取得に失敗しました。' } };
  }
}

/**
 * Handler for /api/m365/test-connection
 */
export async function handleM365TestConnection(reqBody: any): Promise<{ status: number; data: any }> {
  try {
    const { tenantId, clientId, clientSecret, groupEmail, userPrincipalName } = reqBody || {};
...
/**
 * Handler for /api/m365/sync
 */
export async function handleM365Sync(reqBody: any): Promise<{ status: number; data: any }> {
  try {
    const {
      tenantId,
      clientId,
...
/**
 * Handler for /api/m365/send
 */
export async function handleM365Send(reqBody: any): Promise<{ status: number; data: any }> {
  try {
    const { tenantId, clientId, clientSecret, groupEmail, userPrincipalName, toRecipients, ccRecipients, subject, body, isHtml } = reqBody || {};
    ...
    if (!Array.isArray(toRecipients) || toRecipients.length === 0) {
      return { status: 400, data: { success: false, error: '宛先 (toRecipients) を指定してください。' } };
    }
...
/**
 * Handler for /api/m365/attachment
 */
export async function handleM365Attachment(reqBody: any): Promise<{ status: number; data: any }> {
  try {
    const { tenantId, clientId, clientSecret, groupEmail, userPrincipalName, messageId, attachmentId } = reqBody || {};
    ...
    if (!actualTenantId || !actualClientId || !actualClientSecret) {
      return { status: 400, data: { success: false, error: 'Microsoft 365 の認証資格情報が設定されていません。' } };
    }

    if (!messageId || !attachmentId) {
      return { status: 400, data: { success: false, error: 'messageId と attachmentId が必要です。' } };
    }
...
/**
 * Handler for /api/m365/message-attachments
 */
export async function handleM365MessageAttachments(reqBody: any): Promise<{ status: number; data: any }> {
  try {
    const { tenantId, clientId, clientSecret, groupEmail, userPrincipalName, messageId } = reqBody || {};
    ...
    if (!messageId) {
      return { status: 400, data: { success: false, error: 'messageId が必要です。' } };
    }
    ...
    if (!attResp.ok) {
      return { status: 200, data: { success: true, attachments: [] } };
    }
